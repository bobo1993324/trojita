...
template <typename T> class PartVisitor;
...
public:
    ...
    // PartVisitor is owned by PartWalker
    PartWalker(Imap::Network::MsgPartNetAccessManager *manager, result_type *messageView, PartVisitor<result_type> *visitor)
        : m_manager(manager), m_messageView(messageView), m_visitor(visitor)
    { }
    ...
    result_type *walk(const QModelIndex &partIndex,int recursionDepth, const UiUtils::PartLoadingOptions loadingMode)
    {
        ...
        if (wrapInAttachmentView) {
            ...
            if (recognizedMimeType) {
                ...
                if (!isInline) {
                    m_visitor->hideView(contentView);
                }
            }
            ...
            return m_visitor->visitAttachmentPart(0, m_manager, partIndex, m_messageView, contentView);
        }
        ...
        if (mimeType.startsWith(QLatin1String("multipart/"))) {
            ...
            if ((loadingMode & PART_IGNORE_CLICKTHROUGH) || (loadingMode & PART_IGNORE_LOAD_ON_SHOW) ||
                ...
                return m_visitor->visitSimplePartView(0, m_manager, partIndex, m_messageView);
            } else {
                return m_visitor->visitLoadablePart(0, m_manager, partIndex, this, recursionDepth + 1,
...
private:
    ...
    result_type *m_messageView;
    PartVisitor<T> *m_visitor;
